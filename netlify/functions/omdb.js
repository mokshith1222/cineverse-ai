...
async function callOmdb(params, key, label) {
...
export async function handler(event) {
  ...
  try {
    const primaryKey = process.env.PRIMARY_MOVIE_API_KEY || process.env.OMDB_API_KEY;
    const tvdbKey = process.env.TVDB_API_KEY;
    ...
    if (shouldUseBackup && tvdbKey) {
      ...
      if (tvdbBody) {
        console.info('[TVDB function] fallback success');
        return {
          statusCode: 200,
          headers: {
            ...headers,
          ...
          body: JSON.stringify(tvdbBody),
        };
      } else {
        console.error('[TVDB function] fallback failure');
      }
    }
